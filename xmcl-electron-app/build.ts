...
import { copy, emptyDir, ensureFile, ensureDir } from 'fs-extra'
...
async function start() {
  ...
  const config: Configuration = {
    ...
    async afterPack(context) {
      ...
      const dest = `${outDir}/app-${version}-${platformName}.asar`
      const gzipDest = dest + '.gz'
      let src = join(context.appOutDir, 'resources/app.asar')
      if (!existsSync(src)) {
        src = join(context.appOutDir, 'Modula Launcher.app/Contents/Resources/app.asar')
    ...
    async artifactBuildCompleted(context) {
      ...
      if (context.target && context.target.name === 'appx') {
        await buildAppInstaller(version, path.join(__dirname, './build/output4/modula.appinstaller'), electronBuilderConfig.appx!.publisher!)
      }
    },
  }
