      ...
      try {
        ...
        if (
          ...
          if (file.override) {
            ...
            const fileLike = await lookupFile(filePath).catch((e) => {
              this.warn(`Failed to lookup metadata for ${file.path}: ${e.message}`)
              return undefined
            })
            if (!fileLike) {
              addAsOverride(filePath, file.path)
            ...
            if (fileLike.modrinth) {
              ...
              const result = {
                ...
                hashes: {
                  ...
                  sha512: await this.worker.checksum(filePath, 'sha512').catch(() => ''),
                },
              ...
              if (availableDownloads.length === 0 || !result.hashes.sha512) {
