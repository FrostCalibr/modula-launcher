...
export class LaunchService extends AbstractService implements ILaunchService {
  ...
  async #generateOptions(options: LaunchOptions, version: ResolvedVersion, accessToken?: string) {
    ...
    const minMemory: number | undefined = options.minMemory
    let maxMemory: number | undefined = options.maxMemory

    if (!maxMemory || maxMemory <= 0) {
      const totalRam = require('os').totalmem() / 1024 / 1024 // in MB
      if (totalRam >= 16384) {
        maxMemory = 6144 // 6GB for 16GB+ systems
      } else if (totalRam >= 8192) {
        maxMemory = 4096 // 4GB for 8GB+ systems
      } else {
        maxMemory = 2048 // 2GB fallback for low-end systems
      }
      this.log(`Modula: No memory limit specified. Applying smart default: ${maxMemory}MB (Total System RAM: ${Math.floor(totalRam)}MB)`)
    }

    const launcherName = `Modula MC Launcher (${this.app.version})`
    ...
    const getAddress = async () => {
      const port = await this.app.serverPort
      return `http://localhost:${port}/yggdrasil`
    }
    ...
    if (launchOptions.yggdrasilAgent) {
      ...
      let server = launchOptions.yggdrasilAgent.server
      
      // Handle AUTHORITY_DEV (x://dev) or any legacy placeholders
      if (server === AUTHORITY_DEV || server.includes('x://dev') || server.includes('25555')) {
        server = getAddress(port)
        this.log(`Modula: Resolved Yggdrasil server to ${server}`)
      }
      
      launchOptions.yggdrasilAgent.server = server
      
      // Removed debug flag for production stability
      ...
      if (auth?.authlibInjector) {
        let injected = JSON.stringify(auth.authlibInjector)
        // Aggressively replace the hardcoded port in prefetched metadata
        injected = injected.replace(/localhost:25555/g, `localhost:${port}`).replace(/127\.0\.0\.1:25555/g, `127.0.0.1:${port}`)
        const injectedBase64 = Buffer.from(injected).toString('base64')
    ...
    // Apply Smart JVM Optimization
    const settings = await this.app.registry.get(kSettings)
    if (settings.globalOptimizeJVM) {
      if (!launchOptions.extraJVMArgs) launchOptions.extraJVMArgs = []
      ...
      for (const flag of optimizeFlags) {
        const flagName = flag.split('=')[0]
        if (!launchOptions.extraJVMArgs.some(v => v.startsWith(flagName))) {
          launchOptions.extraJVMArgs.push(flag)
        }
      }
      this.log('Applied Modula Smart JVM Optimization flags')
    }
  ...
  async generateArguments(options: LaunchOptions) {
    try {
      if (options.side === 'client') {
        ...
        // Final Global Safety Net: Replace any 25555 in the final args array
        const port = await this.app.serverPort
        const finalArgs = args.map(arg => {
          if (arg.includes('25555')) {
            const newArg = arg.replace(/25555/g, port.toString())
            this.log(`Modula: Global port override in arg: ${arg} -> ${newArg}`)
            return newArg
          }
          return arg
        })
  ...
  async launch(options: LaunchOptions) {
    try {
      ...
      if ('inheritances' in version) {
        ...
        // 🛡️ MODULA FINAL GATE: Force Port Sync before spawning
        if (op.yggdrasilAgent) {
          const port = await this.app.serverPort
          this.log(`[Modula] Universal Port Lockdown: Redirecting to ${port}`)
          const getAddress = (p: number) => `http://localhost:${p}/yggdrasil`
          
          if (op.yggdrasilAgent.server === AUTHORITY_DEV || op.yggdrasilAgent.server.includes('x://dev') || op.yggdrasilAgent.server.includes('25555')) {
            op.yggdrasilAgent.server = getAddress(port)
          }
          
          if (op.extraJVMArgs) {
            op.extraJVMArgs = op.extraJVMArgs.map(arg => arg.replace(/25555/g, port.toString()))
          }
        }
