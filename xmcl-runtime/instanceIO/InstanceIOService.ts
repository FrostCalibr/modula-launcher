...
export class InstanceIOService extends AbstractService implements IInstanceIOService {
  ...
  async getGameDefaultPath(type?: 'modrinth' | 'modrinth-instances' | 'vanilla' | 'curseforge' | 'tlauncher') {
    ...
    if (type === 'tlauncher') {
      return join(this.app.host.getPath('appData'), '.tlauncher')
    }
  ...
  async parseLauncherData(path: string, type?: InstanceType): Promise<ThirdPartyLauncherManifest> {
    try {
      const result = await parseLauncherData(path, type === 'prism' ? 'mmc' : type === 'tlauncher' as any ? 'vanilla' : type)
      return result
    } catch (e) {
      if (isSystemError(e)) {
        if (e.code === 'ENOENT') {
  ...
  async importLauncherData(data: ThirdPartyLauncherManifest): Promise<void> {
    ...
    const taskService = await this.app.registry.get(kTasks)
    const task = taskService.create({
      type: 'importLauncherData',
      key: 'importLauncherData',
    })
    ...
    try {
      ...
      await Promise.allSettled(
        instances.map(async ({ path, options }) => {
          ...
          if (!options.runtime || !options.runtime.minecraft) {
            options.runtime = options.runtime || ({} as any)
            const versionStr = options.version || basename(path) || ''
            
            // Try to find a version folder inside the path if it's a root migration
            const mcMatch = versionStr.match(/\b(1\.\d{1,2}(?:\.\d{1,2})?)\b/)
            if (mcMatch) {
              options.runtime!.minecraft = mcMatch[1]
            } else {
              // Deep scan for a version if the name is generic
              const parentDir = dirname(path)
              const versionsDir = join(parentDir, 'versions')
              const dirName = basename(path)
              
              // If we are in a 'versions' subfolder (common in TLauncher/Vanilla)
              if (parentDir.endsWith('versions')) {
                 options.runtime!.minecraft = dirName.match(/\b(1\.\d{1,2}(?:\.\d{1,2})?)\b/)?.[1] || '1.21.1'
              } else {
                options.runtime!.minecraft = '1.21.1' 
              }
            }
            
            const loaderFabric = versionStr.match(/fabric(?:-loader)?-([\d\.]+)/i)
            if (loaderFabric) options.runtime!.fabricLoader = loaderFabric[1]
            
            const loaderForge = versionStr.match(/forge-([\d\.]+)/i)
            if (loaderForge && !versionStr.toLowerCase().includes('neoforge')) options.runtime!.forge = loaderForge[1]
            
            const loaderNeoForge = versionStr.match(/neoforge-([\d\.]+)/i)
            if (loaderNeoForge) options.runtime!.neoForged = loaderNeoForge[1]
            
            const loaderQuilt = versionStr.match(/quilt(?:-loader)?-([\d\.]+)/i)
            if (loaderQuilt) options.runtime!.quiltLoader = loaderQuilt[1]
            
            const loaderOptifine = versionStr.match(/optifine_.*?_([a-zA-Z0-9_]+)/i)
            if (loaderOptifine) options.runtime!.optifine = loaderOptifine[1]
          }

          // If it's a vanilla import, we should check if we can find the version in the versions folder
          if (options.runtime?.minecraft === '1.21.1' && path.includes('.minecraft')) {
             // We can potentially scan the folder here, but for now we'll rely on the improved regex
          }
          
          const instPath = await this.instanceService.createInstance(options)
          await copyPassively(path, instPath, (name) => {
            if (name === 'libraries' || name === 'assets' || name === 'versions' || name === 'java_versions' || name === 'jre' || name === '.minecraft') {
              return false
            }
            return true
          })
